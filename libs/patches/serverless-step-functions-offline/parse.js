...
module.exports = {
    getRawConfig() {
        ...
        const serverlessConfigurationFileName = this.serverless.config.configurationFilename;
        if (!serverlessPath) {
        ...
        if(serverlessConfigurationFileName !== undefined) {
          manifestFilenames = [ serverlessConfigurationFileName ]
        }
        ...
        return this.getRawConfig()
            .then((serverlessFileParam) => {
                ...
                this.serverless.service.stepFunctions.stateMachines
                    = this.serverless.configurationInput.stepFunctions
                && this.serverless.configurationInput.stepFunctions.stateMachines
                        ? this.serverless.configurationInput.stepFunctions.stateMachines : {};
                this.serverless.service.stepFunctions.activities
                    = this.serverless.configurationInput.stepFunctions.stepFunctions
                && this.serverless.configurationInput.stepFunctions.activities
                        ? this.serverless.configurationInput.stepFunctions.activities : [];
