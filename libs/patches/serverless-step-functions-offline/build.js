...
module.exports = {
    ...
    _states(currentState, currentStateName) {
        ...
        case 'Task': // just push task to general array
            ...
            let fName = this.variables[currentStateName];
            let f = this.functions[fName];
            if (!f) {
              f = this.functions.filter(funct => funct[fName])[0][fName];
              if(!f) {
            ...
            let servicePackage;
            if(f.environment.servicePackage !== undefined) {
              const servicePackage = path.join(this.location, f.environment.servicePackage);
            }
            const buildPath = process.env['FUNCTIONS_BUILD_PATH'];
            let offlineExecutionHandler;
            if(f.environment.offlineExecutionHandler !== undefined) {
              offlineExecutionHandler = path.join(buildPath, f.environment.offlineExecutionHandler);
            }
            else {
              offlineExecutionHandler = path.join(buildPath, f.handler);
            }
            process.env = _.extend(process.env, {"func_handler": f.handler, "func_name": fName, servicePackage})
            result = this._findFunctionPathAndHandler(offlineExecutionHandler);
            ...
            return {
              ...
              f: () => require(path.join(this.location, result.filePath))[result.handler]
            };
