...
const invokeResource = async (request, context) => {
  ...
  const childProcess = execa_1.default(
    'java',
    [
      '-jar',
      '-agentlib:jdwp=transport=dt_socket,server=y,suspend=y,address=5005',
      path_1.default.join(
        amplify_cli_core_1.pathManager.getAmplifyPackageLibDirPath(constants_1.packageName),
        constants_1.relativeShimJarPath
      ),
      path_1.default.join(request.srcRoot, request.package),
      handlerClassName,
      handlerMethodName,
    ],
    {
